...
/// The `http.request.method` field added to the span by [`rquest_otel_span`]
pub const HTTP_REQUEST_METHOD: &str = "http.request.method";
/// The `url.scheme` field added to the span by [`rquest_otel_span`]
pub const URL_SCHEME: &str = "url.scheme";
/// The `server.address` field added to the span by [`rquest_otel_span`]
pub const SERVER_ADDRESS: &str = "server.address";
/// The `server.port` field added to the span by [`rquest_otel_span`]
pub const SERVER_PORT: &str = "server.port";
/// The `url.full` field added to the span by [`rquest_otel_span`]
pub const URL_FULL: &str = "url.full";
/// The `user_agent.original` field added to the span by [`rquest_otel_span`]
pub const USER_AGENT_ORIGINAL: &str = "user_agent.original";
/// The `otel.kind` field added to the span by [`rquest_otel_span`]
pub const OTEL_KIND: &str = "otel.kind";
/// The `otel.name` field added to the span by [`rquest_otel_span`]
pub const OTEL_NAME: &str = "otel.name";
/// The `otel.status_code` field added to the span by [`rquest_otel_span`]
pub const OTEL_STATUS_CODE: &str = "otel.status_code";
/// The `http.response.status_code` field added to the span by [`rquest_otel_span`]
pub const HTTP_RESPONSE_STATUS_CODE: &str = "http.response.status_code";
/// The `error.message` field added to the span by [`rquest_otel_span`]
...
///
/// Check out [`rquest_otel_span`] documentation for examples.
///
...
/// Populates default success/failure fields for a given [`rquest_otel_span!`] span.
#[inline]
...
/// Populates default success fields for a given [`rquest_otel_span!`] span.
#[inline]
...
/// Populates default failure fields for a given [`rquest_otel_span!`] span.
#[inline]
...
impl ReqwestOtelSpanBackend for SpanBackendWithUrl {
    fn on_request_start(req: &Request, ext: &mut Extensions) -> Span {
        ...
        let span = rquest_otel_span!(name = name, req, url.full = %url);
        #[cfg(feature = "deprecated_attributes")]
        {
...
/// let rquest_client = rquest::Client::builder().build()?;
/// let client = ClientBuilder::new(rquest_client)
///    // Inserts the extension before the request is started
...
impl OtelPathNames {
    ...
    /// ```
    /// # use rquest_tracing::OtelPathNames;
