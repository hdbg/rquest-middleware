#[macro_export]
/// [`rquest_otel_span!`](crate::rquest_otel_span) creates a new [`tracing::Span`].
...
/// The second argument passed to [`rquest_otel_span!`](crate::rquest_otel_span) is a reference to an [`rquest::Request`].
///
/// ```rust
...
/// use rquest_tracing::{
///     default_on_request_end, rquest_otel_span, ReqwestOtelSpanBackend
/// };
/// use tracing::Span;
///
...
///         rquest_otel_span!(name = "rquest-http-request", req)
///     }
///
...
/// ```
///
/// If nothing else is specified, the span generated by `rquest_otel_span!` is identical to the one you'd
...
///
/// ```rust,should_panic
/// use rquest_tracing::rquest_otel_span;
...
/// rquest_otel_span!(name = "rquest-http-request", request, time_elapsed = tracing::field::Empty);
///
/// // Define a `name` field with a known value, `AppName`.
/// rquest_otel_span!(name = "rquest-http-request", request, name = "AppName");
///
/// // Define an `app_id` field using the variable with the same name as value.
/// let app_id = "XYZ";
/// rquest_otel_span!(name = "rquest-http-request", request, app_id);
///
/// // All together
/// rquest_otel_span!(name = "rquest-http-request", request, time_elapsed = tracing::field::Empty, name = "AppName", app_id);
/// ```
///
...
/// rquest_otel_span!(level = level, name = "rquest-http-request", request);
/// ```
///
///
/// [`DefaultSpanBackend`]: crate::rquest_otel_span_builder::DefaultSpanBackend
/// [`SpanBackendWithUrl`]: crate::rquest_otel_span_builder::DefaultSpanBackend
/// [`default_on_request_success`]: crate::rquest_otel_span_builder::default_on_request_success
/// [`default_on_request_failure`]: crate::rquest_otel_span_builder::default_on_request_failure
/// [`default_on_request_end`]: crate::rquest_otel_span_builder::default_on_request_end
    ...
    (level=$level:expr, name=$name:expr, $request:ident, $($field:tt)*) => {
        {
            ...
            match $level {
                $crate::rquest_otel_span_macro::private::Level::TRACE => {
                    $crate::request_span!($crate::rquest_otel_span_macro::private::Level::TRACE, method, scheme, host, host_port, user_agent, otel_name, $($field)*)
                },
                $crate::rquest_otel_span_macro::private::Level::DEBUG => {
                    $crate::request_span!($crate::rquest_otel_span_macro::private::Level::DEBUG, method, scheme, host, host_port, user_agent, otel_name, $($field)*)
                },
                $crate::rquest_otel_span_macro::private::Level::INFO => {
                    $crate::request_span!($crate::rquest_otel_span_macro::private::Level::INFO, method, scheme, host, host_port, user_agent, otel_name, $($field)*)
                },
                $crate::rquest_otel_span_macro::private::Level::WARN => {
                    $crate::request_span!($crate::rquest_otel_span_macro::private::Level::WARN, method, scheme, host, host_port, user_agent, otel_name, $($field)*)
                },
                $crate::rquest_otel_span_macro::private::Level::ERROR => {
                    $crate::request_span!($crate::rquest_otel_span_macro::private::Level::ERROR, method, scheme, host, host_port, user_agent, otel_name, $($field)*)
                },
            }
        }
    }
...
pub mod private {
    ...
    macro_rules! request_span {
        ($level:expr, $method:expr, $scheme:expr, $host:expr, $host_port:expr, $user_agent:expr, $otel_name:expr, $($field:tt)*) => {
            $crate::rquest_otel_span_macro::private::span!(
                $level,
                "HTTP request",
